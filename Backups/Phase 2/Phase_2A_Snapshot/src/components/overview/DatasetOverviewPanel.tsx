...
export const DatasetOverviewPanel: React.FC<DatasetOverviewPanelProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<'submissions' | 'schema' | 'diagnostics' | 'roadmap'>('submissions');
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-6 animate-fade-in">
        ...
        <div className="space-y-1.5">
          <div className="flex flex-wrap items-center gap-2.5">
            ...
            <span className="text-[11px] font-bold bg-forest-100 text-forest-900 px-2.5 py-0.5 rounded-full border border-forest-300">
              Phase 2A Ingestion Active
            </span>
          </div>
          <p className="text-xs text-earth-600">
            Ingested <span className="font-semibold text-earth-900">{dataset.comments.length}</span> consultation inputs from file{' '}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
        ...
        <div className="bg-white rounded-xl p-4 border border-sand-300 shadow-subtle space-y-1">
          ...
          <p className="text-[11px] text-earth-600">
            {dataset.diagnostics.droppedEmptyRows > 0 ? `${dataset.diagnostics.droppedEmptyRows} blank rows omitted` : '100% valid rows parsed'}
          </p>
        </div>

        {/* KPI 2: Cleanliness Score */}
        <div className="bg-white rounded-xl p-4 border border-sand-300 shadow-subtle space-y-1">
          <div className="flex items-center justify-between">
            <span className="text-[11px] font-bold uppercase tracking-wider text-earth-500">
              Cleanliness Score
          ...
          <div className="w-full bg-sand-200 h-1.5 rounded-full overflow-hidden">
            <div
              ...
              style={{ width: `${dataset.diagnostics.dataCleanlinessScore}%` }}
            />
          </div>
        </div>

        {/* KPI 3: Stakeholder Diversity */}
        <div className="bg-white rounded-xl p-4 border border-sand-300 shadow-subtle space-y-1">
          <div className="flex items-center justify-between">
            <span className="text-[11px] font-bold uppercase tracking-wider text-earth-500">
              Stakeholder Groups
        ...
        {/* KPI 4: Policy Section References */}
        <div className="bg-white rounded-xl p-4 border border-sand-300 shadow-subtle space-y-1">
          <div className="flex items-center justify-between">
            <span className="text-[11px] font-bold uppercase tracking-wider text-earth-500">
              Sections / Clauses
          ...
          <p className="text-[11px] text-earth-600 truncate">
            {Object.keys(dataset.diagnostics.policySectionCounts).slice(0, 2).join(', ')}...
          </p>
        </div>
        ...
        <div className="bg-white rounded-xl p-4 border border-sand-300 shadow-subtle space-y-1">
          ...
          <p className="font-display font-extrabold text-2xl text-forest-950">
            {dataset.diagnostics.avgCommentWordCount}
          </p>
          <p className="text-[11px] text-earth-600">
            Min: {dataset.diagnostics.minWordCount} · Max: {dataset.diagnostics.maxWordCount} words
          </p>
        </div>
      </div>

      {/* Navigation Tabs */}
      <div className="flex border-b border-sand-300 bg-sand-100/70 px-4 rounded-xl">
        <button
          type="button"
        ...
        >
          <Database className="w-3.5 h-3.5 text-forest-700" />
          <span>Schema & Column Profiles ({dataset.columnProfiles.length})</span>
        </button>
        ...
        >
          ...
          <span>Phase 2 Pipeline Roadmap</span>
        </button>
      </div>
      ...
      {/* Tab 4: Phase 2 Pipeline Roadmap */}
      {activeTab === 'roadmap' && (
        <div className="bg-white rounded-xl p-6 border border-sand-300 shadow-card space-y-6">
          <div className="pb-3 border-b border-sand-200 space-y-1">
            <h3 className="font-display font-bold text-base text-forest-950 flex items-center space-x-2">
              ...
              <span>Phase 2 Implementation Architecture & Next Steps</span>
            </h3>
            <p className="text-xs text-earth-600">
              Phase 2 transforms consultation feedback analysis into a decision intelligence platform across 8 progressive pipeline stages.
            </p>
          </div>
          ...
          <div className="space-y-3 text-xs">
            <div className="p-4 rounded-xl bg-forest-50 border-2 border-forest-700 text-forest-950 flex items-start justify-between gap-3">
              <div className="space-y-1">
                <div className="flex items-center space-x-2 font-bold text-sm text-forest-900">
                  ...
                  <span>Phase 2A — Core & Data Ingestion Pipeline</span>
                </div>
              ...
              <span className="px-2.5 py-1 rounded bg-forest-800 text-white font-bold text-[10px] flex-shrink-0">
                Current Build: Complete & Verified
              </span>
            </div>

            <div className="p-4 rounded-xl bg-white border border-sand-300 text-earth-800 flex items-start justify-between gap-3">
              <div className="space-y-1">
                <span className="font-bold text-earth-900 block">Phase 2B — Dynamic Topic Discovery & Multi-Clause Stance</span>
                <p className="text-earth-600 text-[11px]">
                  Unsupervised topic discovery, root cause classification, and multi-clause stance scoring without fixed lexicon boundaries.
                </p>
              </div>
            ...
            <div className="p-4 rounded-xl bg-white border border-sand-300 text-earth-800 flex items-start justify-between gap-3">
              <div className="space-y-1">
                <span className="font-bold text-earth-900 block">Phase 2C — Multi-Factor Priority Engine</span>
                <p className="text-earth-600 text-[11px]">
                  Deterministic urgency scoring combining sentiment intensity, submission density, and risk keywords.
                </p>
              </div>
              <span className="px-2.5 py-1 rounded bg-sand-200 text-earth-800 font-semibold text-[10px] flex-shrink-0">
                Upcoming (2C)
            ...
            <div className="p-4 rounded-xl bg-white border border-sand-300 text-earth-800 flex items-start justify-between gap-3">
              <div className="space-y-1">
                ...
                <p className="text-earth-600 text-[11px]">
                  Stakeholder cross-tabulation, grounded AI insights, evidence-linked policy recommendations, and PDF dossiers.
                </p>
              </div>
              <span className="px-2.5 py-1 rounded bg-sand-200 text-earth-800 font-semibold text-[10px] flex-shrink-0">
                Upcoming (2D-2G)
