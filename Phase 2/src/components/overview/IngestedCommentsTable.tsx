...
export const IngestedCommentsTable: React.FC<IngestedCommentsTableProps> = ({ dataset }) => {
  ...
  const [sortBy, setSortBy] = useState<'index' | 'length-desc' | 'length-asc' | 'stakeholder' | 'sentiment'>('index');
    ...
    return dataset.comments
      .filter((c) => {
        ...
        if (searchQuery.trim()) {
          ...
          const matchesTopic = c.topics?.some((t) => t.toLowerCase().includes(q)) || false;
          if (!matchesText && !matchesId && !matchesOrg && !matchesTopic) return false;
        }
      ...
      .sort((a, b) => {
        ...
        if (sortBy === 'sentiment') return (b.sentiment?.polarityScore || 0) - (a.sentiment?.polarityScore || 0);
        return 0;
      });
  }, [dataset, searchQuery, selectedStakeholder, selectedSection, selectedSentiment, filterDuplicatesOnly, sortBy]);
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-4 border-b border-sand-200">
        <div>
          <h3 className="font-display font-bold text-base text-forest-950 flex items-center space-x-2">
            <span>Submissions & Traceability Explorer</span>
          ...
          <p className="text-xs text-earth-600 mt-0.5">
            Search, filter, and inspect cleaned consultation inputs with associated topic clusters and sentiment polarity
          </p>
        </div>
        ...
        <div className="relative w-full md:w-72">
          ...
          <input
            ...
            placeholder="Search comments, topics, orgs, IDs..."
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-5 gap-3 text-xs">
        ...
        <div>
          ...
          >
            ...
            <option value="Positive">Positive / Favorable</option>
            <option value="Neutral">Neutral / Procedural</option>
            <option value="Negative">Critical / Opposed</option>
          </select>
        </div>
        ...
        <div>
          ...
          >
            ...
            <option value="sentiment">Highest Polarity First</option>
        ...
        <table className="min-w-full divide-y divide-sand-200 text-xs">
          <thead className="bg-sand-100 text-earth-800 font-bold uppercase tracking-wider text-[10px]">
            <tr>
              ...
              <th className="px-3 py-2.5 text-left w-40">Stakeholder</th>
              <th className="px-3 py-2.5 text-left w-36">Topic & Section</th>
              <th className="px-3 py-2.5 text-left">Cleaned Comment Text</th>
              <th className="px-3 py-2.5 text-center w-28">Sentiment</th>
                  ...
                  <td className="px-3 py-2.5">
                    <div className="space-y-1">
                      ...
                      {comment.stakeholder.organization && (
                        <p className="text-[10px] text-earth-600 font-medium truncate max-w-[140px]">
                          {comment.stakeholder.organization}
                        </p>
                      )}
                    </div>
                  ...
                  <td className="px-3 py-2.5">
                    <div className="space-y-1">
                      {comment.topics && comment.topics.length > 0 && (
                        <span className="inline-flex items-center space-x-1 px-2 py-0.5 rounded text-[10px] font-bold bg-forest-50 text-forest-900 border border-forest-200 truncate max-w-[150px]">
                          <Sparkles className="w-2.5 h-2.5 text-forest-700 flex-shrink-0" />
                          <span className="truncate">{comment.topics[0]}</span>
                        </span>
                      )}
                      <span className="font-semibold text-earth-700 text-[11px] block truncate max-w-[140px]">
                        {comment.policyTarget.section || 'General Policy'}
                      </span>
                    </div>
                  </td>
                  <td className="px-3 py-2.5">
                  ...
                  <td className="px-3 py-2.5 text-center">
                    {comment.sentiment && (
                      <div className="space-y-0.5 inline-block">
                        <span className={`inline-block px-2 py-0.5 rounded text-[10px] font-bold border ${getSentimentBadgeClass(comment.sentiment.label)}`}>
                          {comment.sentiment.label}
                        </span>
                        <span className="block font-mono text-[10px] text-earth-500">
                          {comment.sentiment.polarityScore >= 0 ? '+' : ''}{comment.sentiment.polarityScore}
                        </span>
                      </div>
                    )}
                  </td>
          ...
          <div className="bg-[#FAF8F3] rounded-2xl max-w-2xl w-full overflow-hidden shadow-modal border border-sand-300 flex flex-col animate-fade-in">
            ...
            <div className="p-6 space-y-4 text-xs text-earth-900 max-h-[75vh] overflow-y-auto">
              ...
              <div className="flex flex-wrap gap-2">
                ...
                {activeModalComment.topics && activeModalComment.topics.length > 0 && (
                  <span className="px-2.5 py-1 rounded-lg text-xs font-semibold bg-forest-50 text-forest-900 border border-forest-200">
                    Topic: {activeModalComment.topics[0]}
                  </span>
                )}
                {activeModalComment.sentiment && (
                  <span className={`px-2.5 py-1 rounded-lg text-xs font-bold border ${getSentimentBadgeClass(activeModalComment.sentiment.label)}`}>
                    {activeModalComment.sentiment.label} ({activeModalComment.sentiment.polarityScore >= 0 ? '+' : ''}{activeModalComment.sentiment.polarityScore})
                  </span>
                )}
                {activeModalComment.stance && (
                  <span className={`px-2.5 py-1 rounded-lg text-xs font-semibold border ${getStanceBadgeClass(activeModalComment.stance.label)}`}>
                    Stance: {activeModalComment.stance.label}
                  </span>
                )}
              </div>
              ...
              <div className="p-4 rounded-xl bg-white border border-sand-300 space-y-2">
                ...
                <div className="flex items-center space-x-4 pt-2 border-t border-sand-200 text-[11px] text-earth-500">
                  ...
                  <span>Confidence: <strong>{Math.round((activeModalComment.sentiment?.confidence || 0.8) * 100)}%</strong></span>
                  {activeModalComment.timestamp && <span>Date: <strong>{activeModalComment.timestamp}</strong></span>}
                </div>
              </div>

              {/* Salient Extracted Keywords */}
              {activeModalComment.extractedKeywords && activeModalComment.extractedKeywords.length > 0 && (
                <div className="p-3 bg-white rounded-xl border border-sand-300 space-y-1.5">
                  <span className="text-[10px] font-bold uppercase tracking-wider text-earth-500 block">
                    Salient TF-IDF N-Grams
                  </span>
                  <div className="flex flex-wrap gap-1.5">
                    {activeModalComment.extractedKeywords.map((kw, i) => (
                      <span key={i} className="px-2 py-0.5 rounded bg-sand-100 text-earth-800 border border-sand-300 font-mono text-[11px]">
                        {kw}
                      </span>
                    ))}
                  </div>
