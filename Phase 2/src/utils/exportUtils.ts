...
/**
 * Export normalized dataset as standard CSV with Phase 2B/2C fields
 */
export function exportNormalizedDatasetCsv(dataset: ConsultationDataset) {
  const rows = dataset.comments.map((c, i) => {
    const priorityScore = c.priorityScore !== undefined ? c.priorityScore : 0;
...
/**
 * Export full JSON payload with schema mappings, diagnostics, dynamic topics, and priority analysis
 */
export function exportDatasetJson(dataset: ConsultationDataset) {
  const payload = {
    ...
    priorityAnalysis: dataset.priorityAnalysis,
